...
} from "../../src/shared/theme-generator";
...
tokensRoute.post("/enrich", zValidator("json", EnrichSchema), async (c) => {
  ...
  const result: EnrichedScanResult = {
    ...raw,
    colors,
    themeBlock,
    designTokens,
...
${JSON.stringify(palette, null, 2)}`;
  ...
  try {
    ...
    const aiTokens = JSON.parse(raw) as Array<{
      hex: string;
      cssVar: string;
      label: string;
    }>;
